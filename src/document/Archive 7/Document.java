...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for(int i = 0; i < wordArray.length; ++i){
			while ((i<wordArray.length)&& ( (isVowel(wordArray[i])||((wordArray[i]=='e')&&(i!=(wordArray.length - 1)))))){
				if(firstApp){
					++res;
					firstApp = false; 
				}
	...
	public double getFleschScore()
	{
		...
		double res = (206.835d - (1.015d * ((double)getNumWords() / (double)getNumSentences())) - (84.6d * ((double)this.getNumSyllables()/ (double)this.getNumWords())));
	   return res; 
	}
